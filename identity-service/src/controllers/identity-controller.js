...
const resgiterUser = async (req, res) => {
  ...
  try {
    ...
    //limit all account creation attempts
    try {
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      // limit failed authentication attempts only
      try {
        await loginLimiter.consume(`${req.ip}:${email}`);
      } catch (e) {
        logger.error("Login rate limiter unavailable", e)
        throw e // or return 503
      }
    ...
    try {
      await loginLimiter.delete(`${req.ip}:${email}`);
    } catch (e) {
        logger.warn("Failed to reset login rate limiter", e)
      }
